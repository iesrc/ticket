layui.use(['layer', 'form'], function() {
    ...
    $('button[lay-event="add"]').click(function() {
        ...
        var addModal = layer.open({
            ...
            title: '添加公告信息',
            area: ['420px', 'auto'],
            content:  '<form class="layui-form" action="/ticket/news/add">'+
                '<input type="hidden" >' +
                ...
                '<div class="layui-input-block " >' +
                '<textarea  name="content" rows="10" cols="25" required lay-verify="required|content" placeholder="请输入内容" autocomplete="off" class="layui-textarea" ></textarea>' +
                '</div>' +
                '</div>'+
                '<div class="layui-form-item">' +
                '<div class="layui-input-block">' +
        ...
        form.on('submit(formAdd)', function(data){
            ...
            // 提交表单
            $.ajax({
                url: 'http://localhost:8080/ticket/news/add',
                type: 'POST',
                data: data.field,
